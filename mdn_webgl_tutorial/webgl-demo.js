...
function initBuffers(gl) {
  ...
  //   /* reapeat each color four times for the 4 verticies of the face */
...
function loadTexture(gl, url){
   ...
   * we'll update teh texture with the contents of the image.
   */
  const level = 0;
  ...
  image.onload = function(){
     ...
     * vs non power of 2 images so scheck if the image is a 
     * power of 2 in both dimensions.  
     */
    if(isPowerOf2(image.width) && isPowerOf2(image.height)){
      /* Yes it's a pwer of 2. Generate mips. */
      gl.generateMipmap(gl.TEXTURE_2D)
    }else{
      /**
       * No, it's not a power of 2. Turn off mips and set wrapping
       * to clamp to edge.
       */
